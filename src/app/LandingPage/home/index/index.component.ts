...
export class IndexComponent implements OnInit {
  IP_Pattern="^([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\." +
...
itemList = [];
 ...
 constructor(private HttpClient:HttpClient,private formbuilder:FormBuilder,private objOnBoarding : VariablesService, private spinnerService: Ng4LoadingSpinnerService, private modalService: BsModalService, private router:Router, private adm:LoginService, private toasterService: ToasterService ) { 
 ...
 get domainNm() { return this.signupForm.get('domainNm'); }
 get email() { return this.signupForm.get('email'); }
 get CITY() { return this.signupForm.get('CITY'); }
 get RM() { return this.signupForm.get('RM'); }

 get mobile_no() { return this.signupForm2.get('mobile_no'); }
 get otp_code() { return this.signupForm2.get('otp_code'); }
 
 get username() { return this.signupForm3.get('username'); }
 get password() { return this.signupForm3.get('password'); }
 get confirmPassword() { return this.signupForm3.get('confirmPassword'); }

 get termsandcondition() { return this.signupForm2.get('termsandcondition'); }

 get username1() { return this.forgetpassForm.get('username1'); }
 ...
 UAT_help(UAT_Help:any){
  this.modalRef = this.modalService.show(UAT_Help, { backdrop:'static',class: 'modal-lg' });
 }
 ...
 already_Log(alreadylogin:any,signup:any){
  if(localStorage.getItem('id')!=null){
   this.modalRef7 = this.modalService.show(alreadylogin, { backdrop:'static' });  
  }
  else{
   this.modalRef2 = this.modalService.show(signup, { backdrop:'static' }); 
  }
 ...
 Modalforgotpassw(forgotpassw: TemplateRef<any>) {
  this.modalRef3 = this.modalService.show(forgotpassw, { backdrop:'static' });
  try{
     this.modalRef.hide();
   }
   catch(e){}
...
//  Signup function
sign_up() {
  var CurrentTime = formatDate(this.today, 'dd-MM-yyyy hh:mm:ss a', 'en-US', '+0530');
    ...
    this.adm.sign_up(json)
      .subscribe(
        (data: any) => {
          ...
          if (obj.status == true) {
            this.signup_jira()
            this.toastrmsg('success', "Thanks for registering, once your application is approved it would be conveyed to you on mail sign up.");
            this.spinnerService.hide();
...
signup_jira(){
  var CurrentTime = formatDate(this.today, 'yyyy-MM-dd', 'en-US', '+0530');
    ...
    { 
      try{
          this.adm.SendEmailOTP(this.signupForm.value)
          .subscribe(
            (data:any) => {
              var response= data._body; 
              var obj=JSON.parse(response);
    ...
    verifyOtp1() {
      try {
        this.adm.verify_otp(this.signupForm2.value,this.otp_txt_id)
          .subscribe(
            (data: any) => {
              var response = data._body;
              var obj = JSON.parse(response);
    ...
    HowItWork(modal_hwi:any){
      this.modalRef = this.modalService.show(modal_hwi, { backdrop:'static',class:'modal-lg'});
    }
  ...
  onItemSelect(item: any) {
    ...
    {
         if(item.id==1105){sessionStorage.setItem("1105","true");}
         if(item.id==1106){sessionStorage.setItem("1106","true");}
          if(sessionStorage.getItem("1105")=="true" || sessionStorage.getItem("1106")=="true")
          {
    ...
    if(item.id==1107){sessionStorage.setItem("1107","true");}
    if(item.id==1107 ) 
    {
      this.Cms_allShow=true;
...
onGroupDeSelect(items) {
  ...
  OnItemDeSelect(items: any) {
    ...
    {
         if(items.id==1105){sessionStorage.setItem("1105","false");}
         if(items.id==1106){sessionStorage.setItem("1106","false");}
        if(sessionStorage.getItem("1105")=="false" && sessionStorage.getItem("1106")=="false")
        { 
          this.Webservice_Show=false 
        }
        if(sessionStorage.getItem("1105")=="false" && sessionStorage.getItem("1106")=="false" && sessionStorage.getItem("1107")=="false")
        {
          this.Cms_allShow=false;
          this.Webservice_Show=false
          this.Ecollection_Show=false
    ...
    if(items.id==1107) 
    {
         if(items.id==1107){sessionStorage.setItem("1107","false");}
         if(sessionStorage.getItem("1105")=="false" && sessionStorage.getItem("1106")=="false" && sessionStorage.getItem("1107")=="false")
        {
          this.Cms_allShow=false;
          this.Webservice_Show=false
  ...
  openModaldemo(UATconfirm: TemplateRef<any>) {
   ...
   {     
      ...
      this.collection=this.objOnB.AccountNo+' '+this.objOnB.CmsClientCode+' '+this.objOnB.url+' '+this.objOnB.Ip+' '+this.objOnB.Port+' '+this.objOnB.Checksum+' '+this.objOnB.Encryption+' '+this.objOnB.Certificate+' '+this.objOnB.web+' '+this.objOnB.message+' '+this.objOnB.ModeOffered+' '+this.objOnB.noOftransaction+' '+this.objOnB.transactionLimit+' '+this.objOnB.ammountField+' '+this.objOnB.URN1+' '+this.objOnB.URN2+' '+this.objOnB.DiscText+' '+this.objOnB.IFSC_Code+' '+this.objOnB.vertualCode+' '+this.objOnB.refundCode+' '+this.objOnB.Account_no;
      ...
      var inputFields = {
       ...
       // "file1":file
        ...
        this.HttpClient.post<any>('https://developerapi.icicibank.com:8443/api/v2/jira', formData).subscribe(
         (res) => { 
           console.log(res)
           this.modalRef = this.modalService.show(UATconfirm, { backdrop:'static'});
           this.confirmMsg=res['message']
           this.confirmMsg=this.confirmMsg.substring(51,44);
           //this.toastrmsg('success', res['message']);
            this.modalRef4.hide();
         },
         (err) => {console.log('zze',err); console.log('zzz',err.headers)}
       );
   }
  ...
  get_Production(Production,signin){
    if(localStorage.getItem('id')!=null){
      this.modalRef5 = this.modalService.show(Production, { backdrop:'static' });
...
body.set('username', username);
     this.HttpClient.post('https://developer.icicibank.com/rest/fetch-jiraid',body.toString(),options).subscribe(
         (res) => { 
           this.list = res;
         },
         (err) => { this.list = [];}
      ...
      //console.log('inputFields',inputFields);
       ...
       const formData = new FormData();
        ...
        this.HttpClient.post<any>('https://developerapi.icicibank.com:8443/api/v2/jira', formData).subscribe(
         (res) => { 
            // this.toastrmsg('success', res['message']);
            this.modalRef = this.modalService.show(Prodconfirm, { backdrop:'static'});
     ...
     if(this.Suggestion==""){
      ...
      if(localStorage.getItem('id')!=null){
        var json = {
          ...
          this.adm.feedback(json).subscribe(
            (data:any) => {
              var obj = JSON.parse(data._body);
              if(obj.status==true )
              { 
  ...
  inter_submit(){
    var feedback = "User Interested Full Name = "+this.Inter_full_name+" Contact Number ="+this.Inter_contactnumber;
