...
//aapathonSignUpForm
  ...
  //aapathonSignUpForm
   ...
   this.appathonSignupForm = this.formbuilder.group({
    ...
    captainMobileNumber: ["", [Validators.required, Validators.pattern(this.mobnumPattern)]],
    teamEmail: ["", [Validators.required, Validators.email]],
    teamSize: ["", [Validators.required]],
    // memberName: ["", [Validators.required]],
    // memberMobileNumber: ["", [Validators.required, Validators.pattern(this.mobnumPattern)]],
...
//aapathonSignUpForm
  ...
  openModalAppathonSignup(appathonSignup: TemplateRef<any>) {
    this.modalRef2 = this.modalService.show(appathonSignup, { backdrop: 'static' });
    try {
      this.modalRef.hide();
    }
    catch (e) { }
...
appathonSign_up() {
  ...
  try {
    ...
    this.spinnerService.show();
    this.adm.appathon_sign_up(json)
      .subscribe(
        (data: any) => {
          var response = data._body;
          var obj = JSON.parse(response);
          ...
          if (obj.status == true) {
            this.signup_jira()
            this.toastrmsg('success', "Thanks for registering, once your application is approved it would be conveyed to you on mail");
            this.spinnerService.hide();
...
//aapathonSignUpForm
  verifyOtp2() {
    try {
      this.adm.verify_otp(this.appathonSignupForm2.value, this.otp_txt_id)
        .subscribe(
          (data: any) => {
            var response = data._body;
            var obj = JSON.parse(response);
