...
export class LoginComponent implements OnInit {
  ...
  is_res_error:any="";
  constructor( private router:Router, private adm:LoginService, private toasterService: ToasterService, private spinnerService: Ng4LoadingSpinnerService,) { }
  ...
  sessionSet(key, value, expirationInMin = 20) {
   ...
   // Login function
 Login_admin(username:any,password:any) {
  ...
  if(username==''){
   ...
   admin_acccess(username){   
    this.adm.Admin_access(username)
    .subscribe(
      (data:any) => {
        var response= data._body; 
        var obj=JSON.parse(response);
